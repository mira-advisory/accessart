...
```
backend/functions/accessart-api/   THE Lambda (aa-api): index.mjs, app.mjs, routes/, stores/, shared/
backend/functions/aa-mail-forwarder/  inbound @accessart.net mail forwarder
web/                               React + Vite + TS: marketing, marketplace, admin module
mobile/                            Expo (React Native): iOS + Android apps
infra/                             IaC for API Gateway, tables, mail, domains (nothing console-only)
buildguide/                        decisions, architecture, research: the spec of record
```

## Rules

0. **HARD RULE: everything is prefixed `aa`.** Every named AWS resource belonging to this project takes the `aa` prefix: the Lambda is `aa-api` (`aa-api-dev` in dev), tables are `aa_*` (`aa_waitlist`, `aa_dev_waitlist`), buckets/queues/log groups `aa-*`. The API domains are `api.accessart.net` and `dev-api.accessart.net`. Never create an unprefixed resource. Two documented exceptions only: the CloudFormation stacks are named `accessart` and `accessart-dev`, and their auto-generated IAM roles start `accessart-*`, because the `accessart-iam` deploy grant on the `claude-code` user is scoped to `role/accessart-*` (do not "fix" this without also changing that policy in the console).
...
8. **Frontends never `fetch` directly.** Each surface has one hand-written typed API client file; components consume it via TanStack Query hooks. (Bootstrap exception: the landing's single waitlist call, to be folded into the client when it exists.)
9. **Secrets never in the repo.** Not in `config.json`, not in committed `.env` files. Lambda env blocks in `config.json` are non-secret config only; secrets are set in the console and enumerated by key name.
10. **Dev/prod parity.** Two full stacks from ONE template (`infra/template.yaml` with a Stage parameter): stack `accessart` (Stage=prod, main branch, accessart.net, api.accessart.net) and stack `accessart-dev` (Stage=dev, dev branch, dev.accessart.net, dev-api.accessart.net, its own tables and Cognito pool). Env keys must stay symmetric between the prod and dev blocks in `config.json`. Mail receiving and the apex MX are prod-only. Work lands on `dev` first once real users exist; `main` is production.
11. **HARD RULE: never use the em dash.** Not in copy, not in emails, not in docs, not in code comments. Use a period, comma, colon, or semicolon instead. The brand voice is short dry sentences; the em dash is the tell of machine writing and it is banned in this repo.
...
- Brand voice in all user-facing copy: lowercase, dry, short sentences, "not a gallery" energy. No corporate speak, no emoji, no em dashes.
